...
class TestSubscriptionPlans:
    ...
    def test_checkout_pro_plan(self):
        ...
        response = self.session.post(
            f"{BASE_URL}/api/subscription/checkout",
            json={"plan": "pro", "origin_url": "https://point-control-test.preview.emergentagent.com"},
            headers=headers
        )
        assert response.status_code == 200, f"Expected 200, got {response.status_code}: {response.text}"
    ...
    def test_checkout_business_plan(self):
        ...
        response = self.session.post(
            f"{BASE_URL}/api/subscription/checkout",
            json={"plan": "business", "origin_url": "https://point-control-test.preview.emergentagent.com"},
            headers=headers
        )
        assert response.status_code == 200, f"Expected 200, got {response.status_code}: {response.text}"
